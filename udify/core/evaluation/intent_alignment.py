...
def _op_type_name(op: Any) -> str:
    """兼容地取操作类型名（PatchOperation.op_type.name 或 dict['op_type']）。"""
    if hasattr(op, "op_type"):
        return op.op_type.name
    if isinstance(op, dict):
        return str(op.get("op_type", "")).upper()
    return ""


def _op_payload(op: Any) -> dict[str, Any]:
    """兼容地取操作 payload（PatchOperation.payload 或 dict 本身）。"""
    if hasattr(op, "payload") and isinstance(op.payload, dict):
        return op.payload
    if isinstance(op, dict):
        return op
    return {}
...
class IntentAlignmentEvaluator:
    ...
    def __init__(self, llm_client: LLMClient = None):
        self.llm = llm_client
        self.metrics = [
            AlignmentMetric(name, weight, desc) for name, weight, desc in self.DEFAULT_METRICS
        ]
    ...
    def evaluate(
        ...
        # 1. 评估目标达成度
        goal_score = self._evaluate_goal_achievement(original_intent, structured_intent, patch)
        self._update_metric("goal_achievement", goal_score)

        # 2. 评估约束满足度
        constraint_score = self._evaluate_constraint_satisfaction(structured_intent, patch)
        self._update_metric("constraint_satisfaction", constraint_score)

        # 3. 评估参考匹配度
        reference_score = self._evaluate_reference_match(structured_intent, patch)
        self._update_metric("reference_match", reference_score)

        # 4. 评估范围控制度
        scope_score = self._evaluate_scope_control(original_intent, patch)
        self._update_metric("scope_control", scope_score)

        # 5. 使用LLM进行综合评估（如果可用）
        if self.llm and self.llm.is_available():
            llm_score = self._evaluate_with_llm(original_intent, structured_intent, patch)
            # LLM分数作为额外参考，调整总分
            if llm_score > 0:
        ...
        return {
            ...
            "metrics": {
                m.name: {"score": m.score, "weight": m.weight, "details": m.details}
    ...
    def _evaluate_goal_achievement(
        self, intent: Intent, structured: StructuredIntent, patch: CDLPatch
    ) -> float:
        """评估主要目标是否实现"""
        score = 0.0
        details = []
        ...
        op_types = [_op_type_name(op) for op in operations]

        if intent.intent_type.name == "DIFFICULTY_ADJUSTMENT":
            # 检查是否有数值修改操作
            has_numeric_mod = any(t in ["MODIFY_PROPERTY", "MODIFY_ASSET"] for t in op_types)
            if has_numeric_mod:
                score += 0.6
            ...
            has_key_mod = any(
                any(
                    kw in str(_op_payload(op)).lower()
                    for kw in ["health", "damage", "hp", "attack", "life", "maxlife"]
                )
            ...
            # 检查是否有新增操作
            has_add = any(t in ["ADD_NODE", "ADD_ASSET"] for t in op_types)
            if has_add:
                score = 1.0
    ...
    def _evaluate_constraint_satisfaction(
        self, structured: StructuredIntent, patch: CDLPatch
    ) -> float:
        """评估约束条件是否满足"""
    ...
    def _evaluate_reference_match(self, structured: StructuredIntent, patch: CDLPatch) -> float:
        """评估是否体现参考特征"""
    ...
    def _evaluate_scope_control(self, intent: Intent, patch: CDLPatch) -> float:
        """评估修改范围是否合理"""
        operations = patch.operations if hasattr(patch, "operations") else []
