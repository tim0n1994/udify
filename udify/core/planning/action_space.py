...
class ActionSpace:
    ...
    def generate_actions(self, state: PlanState) -> List[PatchOperation]:
        ...
        if "modify" in intent.description.lower() or "change" in intent.description.lower() or "update" in intent.description.lower():
    ...
    def _generate_add_actions(
        ...
        for node_type in node_types[:5]:  # 限制每种类型的数量
            ...
            actions.append(create_add_node_op(
                node_id=node_id,
                node_type=node_type,
        ...
        if self.enable_add_edge and len(graph.nodes) > 1:
            for _ in range(min(5, len(graph.nodes))):
                ...
                if source.id != target.id:
                    actions.append(create_add_edge_op(
                        source=source.id,
                        target=target.id,
    ...
    def _generate_remove_actions(
        ...
        if self.enable_remove_edge and graph.edges:
            for edge in graph.edges[:max_removals]:
                actions.append(create_remove_edge_op(
                    source=edge.source,
                    target=edge.target,
                    edge_type=edge.type,
                ))
    ...
    def _is_critical_node(self, node: ContentNode) -> bool:
        ...
        if node.type in (NodeType.MECHANIC, NodeType.LEVEL) and "core" in node.name.lower():
            return True
        if "player" in node.name.lower() and node.type == NodeType.CHARACTER:
            return True
        return False
