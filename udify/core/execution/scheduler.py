...
class ExecutionResult:
    ...
    graph_snapshot: ContentGraph | None = None
...
class OperationNode:
    ...
    error: str | None = None
    original_state: dict[str, Any] | None = None  # 用于回滚的原始状态
...
class ExecutionScheduler:
    ...
    def execute_patch(
        self, patch: CDLPatch, target_graph: ContentGraph, validate: bool = True
    ) -> ExecutionResult:
        """
    ...
    def _execute_operation(
        self, operation: PatchOperation, target_graph: ContentGraph
    ) -> dict[str, Any]:
    ...
    def _convert_to_tool_call(
        ...
        if op_type == "add_node":
            ...
            return "graph_remove_node", {"graph": target_graph, "node_id": operation.target_id}
            ...
            return "graph_remove_edge", {"graph": target_graph, "edge_id": operation.target_id}
            ...
            return "asset_remove", {"graph": target_graph, "asset_id": operation.target_id}
...
"""
    ...
    def _validate_result(self, patch: CDLPatch, target_graph: ContentGraph) -> list[str]:
        ...
        for node in target_graph.nodes:
            if not target_graph.edges and not any(
                edge.source == node.id or edge.target == node.id for edge in target_graph.edges
            ):
    ...
    def _rollback_operations(
        ...
        for operation in reversed(executed_operations):
            try:
                ...
                if rollback_op:
                    self._execute_operation(rollback_op, target_graph)
    ...
    def _capture_original_state(
        ...
        if op_type == "remove_node":
            ...
            for edge in target_graph.edges:
                if (
                    getattr(edge, "id", None) == target_id
                    or (getattr(edge, "source", None), getattr(edge, "target", None)) == target_id
                ):
                    return {
                        "edge": {
                ...
                if getattr(edge, "id", None) == target_id:
                    return {
                        "edge": {
                            ...
                            "target": getattr(edge, "target", ""),
                            "type": str(getattr(edge, "type", "")),
                            "properties": dict(getattr(edge, "properties", {})),
                        }
                    }

        elif op_type == "remove_asset" or op_type == "modify_asset":
    ...
    def _create_rollback_operation(
        self, operation: PatchOperation, original_state: dict[str, Any] | None
    ) -> PatchOperation | None:
        """Create rollback operation for given operation using captured original state."""
        op_type = operation.op_type.value

        if op_type == "add_node":
            return PatchOperation(op_type="remove_node", target_id=operation.target_id, payload={})
            ...
            return PatchOperation(op_type="remove_edge", target_id=operation.target_id, payload={})
            ...
            return PatchOperation(op_type="remove_asset", target_id=operation.target_id, payload={})
        ...
        elif op_type == "modify_asset" and original_state and "asset" in original_state:
            return PatchOperation(
    ...
    def _mark_dependents_failed(
        self, failed_op_id: str, failed_operations: list[PatchOperation], errors: list[str]
    ) -> None:
