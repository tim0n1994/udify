...
from .tool_registry import ToolCategory, ToolParameter, ToolRegistry, ToolType
...
def _create_file_tool(registry: ToolRegistry) -> None:
    ...
    def read_file(path: str, encoding: str = "utf-8") -> dict[str, Any]:
        ...
        try:
            with open(path, encoding=encoding) as f:
                content = f.read()
            return {"path": path, "content": content, "size": len(content), "encoding": encoding}
    ...
    def copy_file(source: str, destination: str) -> dict[str, Any]:
        ...
        try:
            ...
            raise RuntimeError(f"Failed to copy file {source} to {destination}: {e}") from e
    ...
    def move_file(source: str, destination: str) -> dict[str, Any]:
        ...
        try:
            ...
            raise RuntimeError(f"Failed to move file {source} to {destination}: {e}") from e
    ...
    def list_files(directory: str, recursive: bool = False, pattern: str = "*") -> dict[str, Any]:
        ...
        try:
            ...
            raise RuntimeError(f"Failed to list files in {directory}: {e}") from e
    ...
    registry.register_tool(
        ...
        tool_type=ToolType.FILE_OPERATION,
        category=ToolCategory.FILE_SYSTEM,
        parameters=[ToolParameter("path", "string", "Path to the file to delete", required=True)],
...
def _create_json_tool(registry: ToolRegistry) -> None:
    ...
    def modify_json(
        path: str, operations: list[dict[str, Any]], create_if_missing: bool = False
    ) -> dict[str, Any]:
        """
        ...
        try:
            ...
            for op in operations:
                ...
                if op_type == "add":
                    ...
                    for key in keys[:-1]:
                        if key not in target:
                            raise KeyError(
                                f"Path not found: {'/'.join(keys[: keys.index(key) + 1])}"
                            )
                        target = target[key]
                    target[keys[-1]] = value
            ...
            return {"path": path, "operations_applied": len(operations), "modified": True}
        except Exception as e:
            raise RuntimeError(f"Failed to modify JSON file {path}: {e}") from e
    ...
    registry.register_tool(
        ...
        parameters=[ToolParameter("path", "string", "Path to JSON file", required=True)],
        executor=read_json,
        safe_mode=True,
    )
    ...
    registry.register_tool(
        ...
        parameters=[
            ...
            ToolParameter(
                "indent", "integer", "Indentation spaces (default: 2)", required=False, default=2
            ),
        ],
        executor=write_json,
        safe_mode=True,
...
def _create_text_tool(registry: ToolRegistry) -> None:
    ...
    def write_text(path: str, content: str, encoding: str = "utf-8") -> dict[str, Any]:
        ...
        try:
            ...
            raise RuntimeError(f"Failed to write text file {path}: {e}") from e

    def replace_text(
        path: str, search: str, replace: str, encoding: str = "utf-8"
    ) -> dict[str, Any]:
        ...
        try:
            ...
            raise RuntimeError(f"Failed to replace text in {path}: {e}") from e
...
def _create_shell_tool(registry: ToolRegistry) -> None:
    ...
    def execute_shell(command: str, cwd: str | None = None, timeout: int = 300) -> dict[str, Any]:
        ...
        try:
            ...
            raise RuntimeError(f"Command timed out after {timeout} seconds: {command}") from None
        except Exception as e:
...
def _create_validation_tool(registry: ToolRegistry) -> None:
    ...
    def validate_json_schema(path: str, schema_path: str) -> dict[str, Any]:
        ...
        try:
            ...
            raise RuntimeError(f"Failed to validate JSON file {path}: {e}") from e
