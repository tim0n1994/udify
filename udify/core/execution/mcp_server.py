...
class McpTool:
    ...
    name: str
    description: str
    inputSchema: dict[str, Any]  # noqa: N815 - MCP spec field name
        ...
        try:
            ...
            if request.jsonrpc != "2.0":
                return self._error_response(
                    request.id, McpErrorCodes.INVALID_REQUEST, "Invalid JSON-RPC version"
                )
            ...
            return self._error_response(None, McpErrorCodes.PARSE_ERROR, f"Invalid JSON: {e}")
        except Exception as e:
            logger.error(f"Error handling MCP request: {e}")
            return self._error_response(
                None, McpErrorCodes.INTERNAL_ERROR, f"Internal server error: {e}"
            )

    def _handle_list_tools(self, request_id: int | None) -> str:
        ...
        try:
            ...
            for tool in all_tools:
                ...
                mcp_tool = McpTool(
                    name=tool["name"], description=tool["description"], inputSchema=input_schema
                )
            ...
            return self._error_response(
                request_id, McpErrorCodes.INTERNAL_ERROR, f"Failed to list tools: {e}"
            )

    def _handle_call_tool(self, request_id: int | None, params: dict[str, Any] | None) -> str:
        """Handle tools/call request."""
        try:
            if not params:
                return self._error_response(
                    request_id, McpErrorCodes.INVALID_PARAMS, "Missing parameters"
                )
            ...
            if not tool_name:
                return self._error_response(
                    request_id, McpErrorCodes.INVALID_PARAMS, "Missing tool name"
                )
            ...
            if not tool_def:
                return self._error_response(
                    request_id, McpErrorCodes.INVALID_PARAMS, f"Tool not found: {tool_name}"
                )
            ...
            if self.use_sandbox and tool_def.metadata.safe_mode:
                ...
                config = SandboxConfig(
                    timeout=tool_def.metadata.timeout, memory_limit="512m", cpu_limit=1.0
                )

                # Create Python script to execute tool
                script = self._generate_tool_script(tool_name, tool_args)
                result = self.sandbox_manager.run_python_script(script=script, config=config)
            ...
            return self._error_response(
                request_id, McpErrorCodes.INTERNAL_ERROR, f"Failed to call tool: {e}"
            )
...
"""
    ...
    def _success_response(self, request_id: int | None, result: Any) -> str:
        """Create success response."""
        response = McpResponse(id=request_id, result=result)
        return json.dumps(asdict(response))

    def _error_response(
        self, request_id: int | None, code: int, message: str, data: Any | None = None
    ) -> str:
