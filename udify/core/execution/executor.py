...
from .scheduler import ExecutionConfig, ExecutionResult, ExecutionScheduler
...
class PatchExecutor:
    ...
    def __init__(
        ...
        execution_config = self.config.execution_config or ExecutionConfig(
            use_sandbox=self.config.use_sandbox, enable_rollback=self.config.enable_rollback
        )
        ...
        self.mcp_server: UdifyMCPServer | None = None
    ...
    def execute_patch(
        ...
        result = self.scheduler.execute_patch(
            patch=patch, target_graph=target_graph, validate=validate
        )

        # Log result
        if result.success:
            logger.info(
                f"Patch execution successful: {len(result.executed_operations)} operations executed"
            )
        else:
            logger.error(
                f"Patch execution failed: {len(result.failed_operations)} operations failed"
            )
    ...
    def validate_patch(self, patch: CDLPatch, target_graph: ContentGraph) -> dict[str, Any]:
        """
            ...
            def run_server():
                ...
                while True:
                    ...
                    client_thread = threading.Thread(
                        target=self._handle_mcp_client, args=(client_socket,)
                    )
    ...
    def register_custom_tool(
        ...
        """
        try:
            from .tool_registry import ToolCategory, ToolParameter, ToolType
