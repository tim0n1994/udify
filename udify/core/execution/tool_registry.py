    ...
    def register_tool(
        ...
        **metadata_kwargs,
        ...
        tool_def = ToolDefinition(
            metadata=metadata, parameters=parameters, executor=executor, validation_fn=validation_fn
        )
        ...
        logger.info(
            f"Tool registered: {name} (type: {tool_type.value}, category: {category.value})"
        )
        ...
        for param_name, param_value in parameters.items():
            ...
            if param_def.constraints:
                if "min" in param_def.constraints and param_value < param_def.constraints["min"]:
                    errors.append(
                        f"Parameter '{param_name}' must be >= {param_def.constraints['min']}"
                    )
                if "max" in param_def.constraints and param_value > param_def.constraints["max"]:
                    errors.append(
                        f"Parameter '{param_name}' must be <= {param_def.constraints['max']}"
                    )
                if (
                    "enum" in param_def.constraints
                    and param_value not in param_def.constraints["enum"]
                ):
                    errors.append(
                        f"Parameter '{param_name}' must be one of {param_def.constraints['enum']}"
                    )
