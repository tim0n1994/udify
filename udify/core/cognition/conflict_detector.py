...
from udify.core.cognition.intent import (
    Intent, StructuredIntent, Constraint, ConstraintType
)
...
class ConflictDetector:
    ...
    def detect(self, intent: Intent, 
                structured_intent: StructuredIntent) -> List[Conflict]:
        """检测意图中的所有冲突
        ...
        for i, goal1 in enumerate(sub_goals):
            for j, goal2 in enumerate(sub_goals):
                ...
                if (goal1.get("target_mechanic") == goal2.get("target_mechanic")):
                    ...
                    # 检查是否对立方向
                    if (type1 in self.OPPOSITE_DIRECTIONS and 
                        type2 == self.OPPOSITE_DIRECTIONS[type1]):
                        conflict = Conflict(
    ...
    def _detect_constraint_conflicts(self, intent: StructuredIntent) -> List[Conflict]:
        ...
        has_easy_constraint = any(
            c.type == ConstraintType.DIFFICULTY and 
            "not too" in c.expression.lower() or "easy" in c.expression.lower()
            for c in constraints
        )
        
        has_hard_reference = any(
            r.name in ["Dark Souls Series", "Elden Ring"] or
            "punishing" in str(r.features).lower() or
            "high_difficulty" in str(r.features).lower()
            for r in references
        )
    ...
    def resolve_conflicts(self, intent: StructuredIntent, 
                         conflicts: List[Conflict]) -> StructuredIntent:
        """尝试自动解决冲突
