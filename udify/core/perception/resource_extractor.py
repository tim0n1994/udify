...
class ResourceExtractor(ABC):
    ...
    def _get_file_type(self, suffix: str) -> str:
        ...
        type_map = {
            ...
            '.psd': 'texture', '.ktx': 'texture', '.pvr': 'texture',
            # 模型
            '.fbx': 'model', '.obj': 'model', '.dae': 'model',
            '.blend': 'model', '.3ds': 'model', '.gltf': 'model',
            '.glb': 'model', '.skp': 'model',
            # 音频
            ...
            # 脚本/代码
            '.cs': 'script', '.js': 'script', '.lua': 'script',
            '.py': 'script', '.gd': 'script', '.cpp': 'script',
            '.h': 'script', '.c': 'script', '.java': 'script',
            # 配置文件
            '.json': 'config', '.xml': 'config', '.yaml': 'config',
            '.yml': 'config', '.ini': 'config', '.cfg': 'config',
            '.txt': 'config', '.csv': 'config',
            # 着色器
            ...
            # 其他
            '.asset': 'unity_asset', '.prefab': 'prefab',
            '.mat': 'material', '.scene': 'scene',
        }
...
class UnrealResourceExtractor(ResourceExtractor):
    ...
    def _create_asset_from_uasset(self, file_path: Path, base_dir: Path) -> Optional[ContentAsset]:
        """从 .uasset 文件创建资产记录"""
        try:
    ...
    def _create_asset_from_file(self, file_path: Path, base_dir: Path, file_type: str) -> Optional[ContentAsset]:
        """从文件创建资产记录"""
        try:
            rel_path = file_path.relative_to(base_dir)
        except ValueError:
    ...
    def _create_raw_asset(self, file_path: Path, base_dir: Path) -> Optional[ContentAsset]:
        """从原始资源文件创建记录"""
        suffix = file_path.suffix
        file_type = self._get_file_type(suffix)
...
class GodotResourceExtractor(ResourceExtractor):
    ...
    def _create_asset(self, file_path: Path, base_dir: Path) -> Optional[ContentAsset]:
        """创建 Godot 资源记录"""
