...
class PerceptionEngine:
    ...
    def __init__(
        self,
        engine_detector: CompositeEngineDetector | None = None,
        resource_extractor: CompositeResourceExtractor | None = None,
        mechanism_analyzer: CompositeMechanismAnalyzer | None = None,
    ):
    ...
    def perceive(self, path: str | Path, media_type: MediaType | None = None) -> ContentGraph:
        """
        感知内容，生成内容图谱
        ...
        try:
            ...
            # 如果是游戏，检测引擎
            engine_result: EngineDetectionResult | None = None
            ...
            if media_type == MediaType.GAME and engine_result:
                extraction_result = self.resource_extractor.extract(path, engine_result.engine)
            else:
                # 通用资源提取
    ...
    def _detect_media_type(self, path: Path) -> MediaType:
        ...
        if path.is_file():
            ...
            data_dirs = [
                f
                for f in files
                if f.is_dir() and any(x in f.name for x in ["Data", "data", "Content", "Resources"])
            ]
    ...
    def _calculate_overall_confidence(
        self, graph: ContentGraph, engine_result: EngineDetectionResult | None
    ) -> float:
        """计算总体置信度"""
        scores = []
...
# 便捷函数
def perceive_content(path: str | Path, media_type: MediaType | None = None) -> ContentGraph:
    """
    便捷函数：感知内容
