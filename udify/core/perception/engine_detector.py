...
class EngineDetector(ABC):
    ...
    @abstractmethod
    def detect(self, path: Path) -> Optional[EngineDetectionResult]:
        """
        检测给定路径是否使用本引擎
...
class UnityDetector(EngineDetector):
                ...
                if data_dir.is_dir():
                    if data_dir.suffix == '.app':
                        ...
                        if resources_data.is_dir():
                            ...
                            confidence += 0.3
                            if any((resources_data / f).exists() for f in ["globalgamemanagers", "globalgamemanagers.assets"]):
                        ...
                        # Windows/Linux
                        if any((data_dir / f).exists() for f in ["globalgamemanagers", "globalgamemanagers.assets"]):
    ...
    def _detect_from_archive(self, path: Path) -> Optional[EngineDetectionResult]:
        """从压缩包中检测 Unity"""
        try:
            if zipfile.is_zipfile(path):
                with zipfile.ZipFile(path, 'r') as zf:
                    ...
                    # Android APK 结构
                    if "assets/bin/Data/Managed/" in [f for f in file_list] or \
                       any("assets/bin/Data/" in f for f in file_list):
    ...
    def _check_unity_fs_header(self, path: Path) -> bool:
        ...
        try:
            with open(path, 'rb') as f:
                header = f.read(16)
                return header.startswith(self.UNITY_FS_SIGNATURE) or \
                       header.startswith(self.UNITY_RAW_SIGNATURE)
        except Exception:
            return False
    
    def _extract_unity_version(self, data_dir: Path) -> Optional[str]:
        """尝试从 globalgamemanagers 或 level0 中提取 Unity 版本"""
        try:
            # Unity 5+ 的版本信息存储在 globalgamemanagers 的前几个字节
            for filename in ["globalgamemanagers", "globalgamemanagers.assets", "level0", "data.unity3d"]:
...
class UnrealDetector(EngineDetector):
    ...
    def _extract_unreal_version(self, uproject_path: Path) -> Optional[str]:
...
class GodotDetector(EngineDetector):
    ...
    def _extract_godot_version(self, project_path: Path) -> Optional[str]:
...
class RPGMakerDetector(EngineDetector):
    ...
    def _extract_rpgmv_version(self, system_json: Path) -> Optional[str]:
    ...
    def _extract_rpgmaker_ini_version(self, ini_path: Path) -> Optional[str]:
...
class GameMakerDetector(EngineDetector):
    ...
    def detect(self, path: Path) -> Optional[EngineDetectionResult]:
        ...
        if path.is_dir():
            ...
            gm_files = list(path.rglob("*.gmk")) + list(path.rglob("*.gm81")) + list(path.rglob("*.gmx"))
            if gm_files:
