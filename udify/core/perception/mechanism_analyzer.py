...
class MechanismAnalyzer(ABC):
    ...
    def _link_nodes(self, graph: ContentGraph, source_id: str, target_id: str, 
                    edge_type: EdgeType = EdgeType.DEPENDS_ON, weight: float = 1.0):
        """在图谱中创建边"""
...
class UnityMechanismAnalyzer(MechanismAnalyzer):
    ...
    def _analyze_scenes(self, graph: ContentGraph) -> None:
        """分析场景文件"""
        scene_assets = [a for a in graph.assets if a.type in ('scene', 'unity_asset') 
                       and 'scene' in a.path.lower()]
        ...
        for asset in scene_assets:
            ...
            related_assets = [a for a in graph.assets 
                            if Path(a.path).parent == scene_dir]
            
            for related in related_assets:
                if related.type in ('texture', 'model', 'audio'):
                    self._link_nodes(graph, level_node.id, related.id, 
                                   EdgeType.CONTAINS)
            ...
            if script_path.exists():
                ...
                for keyword, (cn_name, en_name) in mechanic_keywords.items():
                    if keyword in content:
                        # 检查是否已存在相同机制
                        existing = [n for n in graph.nodes 
                        ...
                        if not existing:
                            ...
                            # 关联到脚本资源
                            resource_nodes = [n for n in graph.nodes 
                                            if n.source_path == asset.path]
                            if resource_nodes:
                                self._link_nodes(graph, mechanic_node.id, 
                                               resource_nodes[0].id, 
                                               EdgeType.REFERENCES)
    ...
    def _analyze_json_config(self, graph: ContentGraph, asset: ContentAsset, 
    ...
    def _extract_player_config(self, graph: ContentGraph, asset: ContentAsset, 
                               data: Dict) -> None:
        """提取玩家配置"""
        # 创建玩家角色节点
        ...
        # 关联到机制
        health_nodes = [n for n in graph.nodes 
                       if n.type == NodeType.MECHANIC and '生命' in n.name]
        if health_nodes:
            self._link_nodes(graph, player_node.id, health_nodes[0].id, 
                           EdgeType.DEPENDS_ON)
    
    def _extract_character_config(self, graph: ContentGraph, asset: ContentAsset, 
                                  data: Dict, char_type: str) -> None:
        """提取角色配置"""
        if isinstance(data, dict):
            # 可能是单个角色
    ...
    def _extract_item_config(self, graph: ContentGraph, asset: ContentAsset, 
                             data: Dict) -> None:
        """提取物品配置"""
    ...
    def _extract_level_config(self, graph: ContentGraph, asset: ContentAsset, 
                              data: Dict) -> None:
        """提取关卡配置"""
        if isinstance(data, dict):
    ...
    def _extract_quest_config(self, graph: ContentGraph, asset: ContentAsset, 
                              data: Dict) -> None:
        """提取任务配置"""
        if isinstance(data, list):
    ...
    def _extract_skill_config(self, graph: ContentGraph, asset: ContentAsset, 
                              data: Dict) -> None:
        """提取技能配置"""
        if isinstance(data, list):
    ...
    def _extract_generic_config(self, graph: ContentGraph, asset: ContentAsset, 
                                data: Dict) -> None:
        """通用配置分析"""
        # 尝试识别数值配置（如平衡参数）
        if isinstance(data, dict):
            numeric_values = {k: v for k, v in data.items() 
    ...
    def _analyze_xml_config(self, graph: ContentGraph, asset: ContentAsset, 
                            config_path: Path) -> None:
        """分析 XML 配置文件"""
        try:
    ...
    def _extract_xml_characters(self, graph: ContentGraph, asset: ContentAsset, 
                                root: ET.Element) -> None:
        """从 XML 提取角色信息"""
        for char_elem in root.findall('.//character') + root.findall('.//player') + root.findall('.//enemy'):
    ...
    def _extract_xml_items(self, graph: ContentGraph, asset: ContentAsset, 
                           root: ET.Element) -> None:
        """从 XML 提取物品信息"""
    ...
    def _extract_xml_levels(self, graph: ContentGraph, asset: ContentAsset, 
                            root: ET.Element) -> None:
        """从 XML 提取关卡信息"""
        for level_elem in root.findall('.//level') + root.findall('.//stage') + root.findall('.//map'):
    ...
    def _analyze_yaml_config(self, graph: ContentGraph, asset: ContentAsset, 
                             config_path: Path) -> None:
        """分析 YAML 配置文件"""
    ...
    def _analyze_resource_dependencies(self, graph: ContentGraph) -> None:
        ...
        for asset in graph.assets:
            ...
            # 查找同目录下的其他资源
            related = [a for a in graph.assets 
                      if Path(a.path).parent == asset_dir and a.path != asset.path]
            ...
            for rel in related:
                ...
                if asset_nodes and rel_nodes:
                    self._link_nodes(graph, asset_nodes[0].id, rel_nodes[0].id,
                                   EdgeType.RELATED_TO)
...
class RPGMakerMechanismAnalyzer(MechanismAnalyzer):
            ...
            if isinstance(data, list):
                for enemy_data in data:
                    ...
                    stats = {
                        ...
                        'mp': enemy_data.get('params', [0, 0])[1] if 'params' in enemy_data and len(enemy_data['params']) > 1 else 0,
                        'attack': enemy_data.get('params', [0, 0, 0])[2] if 'params' in enemy_data and len(enemy_data['params']) > 2 else 0,
                        'defense': enemy_data.get('params', [0, 0, 0, 0])[3] if 'params' in enemy_data and len(enemy_data['params']) > 3 else 0,
                    }
    ...
    def _analyze_maps(self, graph: ContentGraph) -> None:
        ...
        map_assets = [a for a in graph.assets 
                     if a.format == 'json' and 'map' in a.path.lower()]
    ...
    def _analyze_system_config(self, graph: ContentGraph) -> None:
        """分析系统配置"""
        system_assets = [a for a in graph.assets 
                        if 'system' in a.path.lower() and a.format == 'json']
