...
class LearningEngine:
    ...
    async def update_action_weight(
        ...
        if rating is not None:
            weight.average_rating = (weight.average_rating * weight.usage_count + rating) / (
                weight.usage_count + 1
            )

        weight.usage_count += 1

    async def learn_successful_pattern(self, patch: Any, rating: int) -> ModPattern | None:
        """从成功的 Patch 中学习模式"""
        if rating < 4:
            return None
    ...
    async def _analyze_feedback(self, feedback: UserFeedback) -> None:
        ...
        if "patch" in feedback.metadata:
            ...
            if feedback.rating and feedback.rating >= 4:
                await self.learning_engine.learn_successful_pattern(patch, feedback.rating)
