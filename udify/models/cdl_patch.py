...
class PatchOperation:
    ...
    def _hashable_payload(obj: Any) -> Any:
        ...
        if isinstance(obj, dict):
            return tuple(
                (k, PatchOperation._hashable_payload(v))
...
class PatchConflict:
    ...
    operation_a: Optional[PatchOperation]
    operation_b: Optional[PatchOperation]
    description: str
...
class CDLPatch:
    ...
    _original_state: Optional[Dict[str, Any]] = field(default=None, repr=False)
    ...
    def summary(self) -> str:
        ...
        if self.conflicts:
            lines.append(f"  Conflicts: {len(self.conflicts)} ({sum(1 for c in self.conflicts if c.severity == 'error')} errors)")
...
class PatchApplicator:
    ...
    def _apply_operation(self, op: PatchOperation, graph: ContentGraph) -> None:
        ...
        if op.op_type == OpType.ADD_NODE:
            ...
            # 移除相关边
            graph.edges = [e for e in graph.edges if e.source != op.target_id and e.target != op.target_id]
            # 移除节点
